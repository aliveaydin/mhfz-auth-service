...
console.log("Starting mhfz-auth-service service...");
...
const MhfzLoginSession = require("./project-session/mhfz-login-session");

let processActive = false;
let expressServer = null;

const start = async () => {
  process.title = "node-mhfz-auth-service";
  ...
  try {
    ...
    await setRedisData("mhfz-auth-service:start", today.toUTCString());
    console.log(
      "Service start datetime is written to Redis key mhfz-auth-service:start",
      today.toUTCString(),
    );
  } catch (err) {
  ...
  console.log(
    "mhfz-auth-service is listening HTTP/REST port " + servicePort.toString(),
  );
  console.log(
  ...
  // init login database
  const loginSession = new MhfzLoginSession();
...
/* starting the server mhfz-auth-service .... */
start();
