...
const MhfzSession = require("./mhfz-session");

class MhfzLoginSession extends MhfzSession {
  ...
  async relogin(req) {
    console.log("mhfz session found but a relogin is requested");
    try {
      const userField = "id";
  ...
  async setLoginToRequest(req, byPassword, bySubject) {
    ...
    session.checkTokenMark = "mhfz-inapp-token";
    session._USERID = session.userId;
    ...
    const cookieName = `mhfz-access-token`;
...
// Export the class
module.exports = MhfzLoginSession;
