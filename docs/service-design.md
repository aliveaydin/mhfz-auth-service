...
but a more specific purpose of the module is also to store all required configuration to generate an automatic user service for the project which is named 'mhfz-auth-service'.
...
The service uses a **PostgreSQL** database for data storage, with the database name set to `mhfz-auth-service`.
